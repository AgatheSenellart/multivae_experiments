...
def eval_model(path, model, test_set, wandb_path=None):
    ...
    state_dicts = dict(
        image=torch.load(
            os.path.join(classifiers_path, "best_image_classifier_model.pth.tar"),
            map_location=device,
        )["state_dict"],
        audio=torch.load(
            os.path.join(classifiers_path, "best_sound_classifier_model.pth.tar"),
            map_location=device,
        )["state_dict"],
        trajectory=torch.load(
            os.path.join(classifiers_path, "best_trajectory_classifier_model.pth.tar"),
            map_location=device,
        )["state_dict"],
    )
    ...
    coherence_config = CoherenceEvaluatorConfig(
        128, wandb_path=wandb_path, num_classes=10, give_details_per_class=True
    )
    ...
    for m in ["SSIM", "MSE"]:
        ...
        recon_config = ReconstructionConfig(
            batch_size=64, wandb_path=wandb_path, metric=m
        )

        recon_module = Reconstruction(
            model, test_dataset=test_set, output=path, eval_config=recon_config
        )
